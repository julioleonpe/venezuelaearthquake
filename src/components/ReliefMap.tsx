...
export function ReliefMap({
  ...
  // Sync markers whenever the filter or the data changes.
  useEffect(() => {
    ...
    shown.forEach((p) => {
      ...
      const detail =
        p.type === "acopio"
          ? p.needs && `<br><span class="relief-popup__row"><b>Reciben:</b> ${escapeHtml(p.needs)}</span>`
          : p.capacity && `<br><span class="relief-popup__row"><b>Capacidad:</b> ${escapeHtml(p.capacity)}</span>`;
      marker.bindPopup(
        `<strong>${TYPE_GLYPH[p.type]} ${escapeHtml(p.name)}</strong>` +
          (p.verified
            ? ""
            : `<span class="relief-popup__badge">sin verificar</span>`) +
 ...
 */
function reliefIcon(p: ReliefPoint): L.DivIcon {
  const color = TYPE_COLOR[p.type];
  const cls = `relief-marker relief-marker--${p.type}${p.verified ? "" : " relief-marker--unverified"}`;
