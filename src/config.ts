 ...
 *
 * The point data itself is a daily STATIC SNAPSHOT bundled with the app
 * (src/data/relief-points.json, refreshed by scripts/fetch-relief-points.mjs) —
 * not fetched live. The upstream Google Apps Script endpoint 302-redirects to
 * script.googleusercontent.com, and that cross-origin redirect trips the browser's
 * CORS check on a direct fetch; a serverless proxy worked around it but kept
 * failing on Vercel, so we snapshot server-side instead. The data is deliberately
 * kept OUT of the Hub's verification gate — third-party, community-reported,
 * attributed and normalized client-side (see src/lib/acopios.ts), with each
 * point's "sin verificar" state preserved.
 */
